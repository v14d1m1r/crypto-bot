...
type Config = { symbol:string; interval:string; fast_ema:number; slow_ema:number; starting_cash:number; mode:string };
type StatusResponse = { bot:BotStatus|null; config:Config };
...
export default function Home() {
  ...
  const bot=status.bot,pnl=bot?bot.equity-status.config.starting_cash:0,pnlPercent=bot?(pnl/status.config.starting_cash)*100:0;
  ...
  return <main className="shell">
    ...
    <section className="metrics">
      <article><label>NET EQUITY</label><strong>{money(bot?.equity??status.config.starting_cash)}</strong><span className={pnl>=0?'positive':'negative'}>{pnl>=0?'↑':'↓'} {pnlPercent.toFixed(2)}%</span></article>
      <article><label>AVAILABLE CASH</label><strong>{money(bot?.cash??status.config.starting_cash)}</strong><span>25% position allocation</span></article>
